...
class output_var:
    ...
    def write_scenario_needed_results(self, d):
        df = pd.DataFrame({'Date': self.date_range[1:d+1],
                           'Value of statistical life-year (VSL) loss': self.VSL_plot[1:d+1],
                           'Cost of mass tests': self.univ_test_cost[1:d+1],
                           'Cost of contact trace and tests':self.trac_test_cost[1:d+1],
                           'Cost of symptom-based tests': self.bse_test_cost[1:d+1],
                           'Cost of quarantine': self.quarantine_cost_plot[1:d+1],
                           'Total cost of tests': self.tot_test_cost_plot[1:d+1],
                           'Number of new diagnosis through contact trace and tests': self.num_trac[1:d+1],
                           'Number of new diagnosis through symptom-based tests': self.num_base[1:d+1],
                           'Number of new diagnosis through mass tests':self.num_uni[1:d+1],
                           'Newly diagnosed': self.num_inf_plot[1:d+1],
                           'Number of hospitalized per day': self.num_hosp_plot[1:d+1],
                           'Number of deaths per day': self.num_dead_plot[1:d+1],
                           'Number of diagnosed infections': self.num_diag_inf[1:d+1],
                           'Number of undiagnosed infections': self.num_undiag_inf[1:d+1],
                           'Number of new infections': self.num_new_inf_plot[1:d+1],
                           'Number of quarantines (only true positives)': self.num_quarantined_plot[1:d+1],
                           'Number of travel-related infection': self.travel_num_inf_plot[1:d+1],
                           'Number of trace and tests': self.T_c_plot[1:d+1],
                           'Number of mass tests': self.T_u_plot[1:d+1],
                           'Contact rate - average contacts per person per day': self.policy_plot[1:d+1, 0],
                        ...
                        #    'Testing capacity – maximum tests per day through mass tests': self.policy_plot[:, 2],
                           'Percentage through contact trace and tests': self.policy_plot[1:d+1, 1],
                           'Percentage through mass tests': self.policy_plot[1:d+1, 2],
                           'Cumulative diagnosis': self.cumulative_inf[1:d+1],
                           'Cumulative hospitalized': self.cumulative_hosp[1:d+1],
                           'Cumulative deaths': self.cumulative_dead[1:d+1],
                           'Cumulative cases (diagnosed and undiagnosed)': self.cumulative_new_inf_plot[1:d+1],
                           'Cumulative costs': self.cumulative_cost_plot[1:d+1]
