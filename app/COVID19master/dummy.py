...
class CovidModel():
    # Function to initialize variables needed and initialize the simulation
    def __init__(self):
...
# static - static decision choice - the decision won't be changed after you make it
def run_COVID_sim(decision, prog_bar, static = 'N'):
...
# Function to compare different decision choices
# Input parameters:
# State: the State/university you want to model
# a_c_l: contact trace and tests
# a_u_l: mass tests
# a_sd_l: contact rate
def compare_scenarios_mod(State, a_c, a_u, a_sd, cost):
    set_up_COVID_sim(State)

    a_c_l = [float(i) for i in a_c.split(',')]
    a_u_l = [float(i) for i in a_u.split(',')]
    a_sd_l = [float(i) for i in a_sd.split(',')]
    gv.test_cost = [float(i) for i in cost.split(',')]
    scenario = [[j,k,m] for j in a_c_l
                        for k in a_u_l
                        for m in a_sd_l]

    num_scenario = len(scenario)
    prog_bar = IncrementalBar('CODE IS RUNNING: \t', max = num_scenario * gv.T_max * gv.inv_dt)

    tables = int(len(scenario)/len(a_sd_l))
    path_new =  pathlib.Path('results/summary.xlsx')
    writer_new = pd.ExcelWriter(path_new, engine = 'xlsxwriter')
    last_col_df = pd.DataFrame() #creates a new dataframe that's empty

    for n in range(tables):
        table_path = pathlib.Path('results/table%d.xlsx'%(n+1))
        writer = pd.ExcelWriter(table_path, engine = 'xlsxwriter')
        names = []

        for i in scenario[len(a_sd_l)*n: len(a_sd_l) * n + len(a_sd_l)]:
            ...
            model = run_COVID_sim(decision, prog_bar, static = 'Y')

            d = 3                           # infectious duration (in days)
            p = model.tran_prob             # transmission risk
            R0 = p * a_sd * d * (1 - a_c - a_u) # R0 value
            sar = model.second_attack_rate  # second attack rate
            unit_cost = model.cost_tst      # unit cost
            num = model.num_to_init_trace   # number of total diagnosis until trace initiation
            pop_size = model.tot_pop        # total population size
            df = model.op_ob.write_scenario_needed_results()
            df2 = model.op_ob.write_summary_results(df, p, a_sd, 100*a_c, sar, R0, 100*a_u, unit_cost, num,n+1 ,pop_size)

            name = 'c = %d, T = %.f%%, U = %.f%%'%(a_sd, 100*a_c, 100*a_u)
            names.append(name)
            df.to_excel(writer, sheet_name = name)
            last_col_df = last_col_df.append(df2,ignore_index = True)
    ...
    last_col_df.to_excel(writer_new, sheet_name= 'summary')
    writer_new.save()
    return tables
...
def set_up_COVID_sim(State):
    ...
    inv_dt = 10                 # time steps within each day
    init_num_inf = 4            # initial number of infected people
    pop_size = 38000            # total population size
    travel_num_inf = 0.5        # number of infected among travelers
    trans_prob = 0.0249         # transmission risk
    num_to_init_trace = 20      # number of total diagnosis until trace initiation
...
if  __name__ == "__main__":
    ...
    State = 'UMASS'
    print('Please use comma to separate your multiple inputs')
    a_c_l = input('Input contact trace and tests (%): ')
    a_u_l = input('Input mass tests(%): ')
    a_sd_l = input('Input contact rate: ')
    cost = input('Input unit cost (symtom-based, contact trace and tests, mass tests, quarantine): ')

    tables = compare_scenarios_mod(State,a_c_l, a_u_l, a_sd_l, cost)
    for i in range(tables):
        sp.plot_results(i+1)
        im.merge_image_scenario(i+1)
